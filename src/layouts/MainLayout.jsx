...
import { LogOut, BookOpen, Grid, User, LogIn, Bell, CheckCircle2, Clock, X, AlertTriangle, Menu } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <header className="sticky top-0 z-50 w-full border-b border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 backdrop-blur">
        ...
        {/* Mobile Menu */}
        {isMobileMenuOpen && (
          <div className="md:hidden border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-4 py-4 shadow-lg animate-in slide-in-from-top-2">
            <nav className="flex flex-col gap-4">
              <Link to="/" onClick={() => setIsMobileMenuOpen(false)} className="text-sm font-medium hover:text-accent transition-colors py-2">Browse</Link>
              {session && (
                <Link to="/federation" onClick={() => setIsMobileMenuOpen(false)} className="text-sm font-medium hover:text-accent transition-colors flex items-center gap-1 py-2">
                  <Grid className="h-4 w-4" /> The Grid
                </Link>
              )}
              ...
              <div className="pt-4 border-t border-slate-200 dark:border-slate-800 flex flex-col gap-4">
                {session ? (
                  <>
                    <Link to="/dashboard" onClick={() => setIsMobileMenuOpen(false)} className="flex items-center gap-3 py-2">
                      <div className="h-8 w-8 rounded-full bg-slate-100 dark:bg-slate-800 border-2 border-white dark:border-slate-700 overflow-hidden flex items-center justify-center">
                        {user?.avatar_url ? (
                          <img src={user.avatar_url} alt="avatar" className="h-full w-full object-cover" />
                        ) : (
                      ...
                      <span className="text-sm font-medium text-slate-900 dark:text-white">Dashboard</span>
                    </Link>
                    <button onClick={() => { signOut(); setIsMobileMenuOpen(false); }} className="flex items-center gap-3 py-2 text-slate-500 hover:text-red-500 dark:hover:text-red-400 text-sm font-medium transition-colors text-left">
                      <LogOut className="h-4 w-4" /> Sign Out
                    </button>
                  </>
                ) : (
                  <div className="flex flex-col gap-3">
                    <Link to="/auth/login" onClick={() => setIsMobileMenuOpen(false)} className="btn btn-ghost w-full justify-center">Sign In</Link>
                    <Link to="/auth/register" onClick={() => setIsMobileMenuOpen(false)} className="btn btn-primary w-full justify-center flex gap-2">
                      <LogIn className="w-4 h-4"/> Register
                    </Link>
                  </div>
                )}
